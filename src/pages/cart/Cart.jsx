...
function Cart() {
  ...
  const getCheckoutFormHandler = (event) => {
    const name = event.target.name;
    const value = event.target.value;
    setCheckFormData((prev) => ({ ...prev, [name]: value }));
  };
  const buyNow = async () => {
    console.log(checkFormData) 
    if ( Object.values(checkFormData).some((item) => item.trim() === "")) {
      ...
      >
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
          ...
          >
            ...
            getCheckoutFormHandler={getCheckoutFormHandler} 
            buyNow={buyNow} />
          </div>
        </div>
      </div>
